...
function ToolStrip({ activeToolId, activateTool }: { readonly activeToolId: ToolId; readonly activateTool: (toolId: ToolId) => void }) {
  return <nav className="tool-strip" aria-label="Tool strip">{toolRegistry.tools.map((tool, index) => <span key={tool.id} className={index === 5 || index === 7 ? "tool-strip__group" : undefined}><IconButton label={`${tool.label} tool${tool.shortcut ? ` (${tool.shortcut.key.toUpperCase()})` : ""}`} icon={tool.icon} selected={tool.id === activeToolId} onClick={() => activateTool(tool.id)} /></span>)}</nav>;
}
