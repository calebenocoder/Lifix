...
panelRegistry.register({ id: "layers", type: "inspector.layers", title: "Layers", icon: "layers", minimumSize: { width: 220, height: 180 }, preferredSize: { width: 288, height: 340 }, dockable: true, floatable: true, closable: true }, () => <PanelChrome title="Layers" icon="layers"><ScrollArea><p className="panel-placeholder">Layer controls will connect to the Core in a later milestone.</p></ScrollArea></PanelChrome>);
panelRegistry.register({ id: "properties", type: "inspector.properties", title: "Properties", icon: "properties", minimumSize: { width: 220, height: 160 }, preferredSize: { width: 288, height: 280 }, dockable: true, floatable: true, closable: true }, () => <PanelChrome title="Properties" icon="properties"><p className="panel-placeholder">Selection properties are not available yet.</p></PanelChrome>);
panelRegistry.register({ id: "color", type: "inspector.color", title: "Color", icon: "color", minimumSize: { width: 220, height: 140 }, preferredSize: { width: 288, height: 220 }, dockable: true, floatable: true, closable: true }, () => <PanelChrome title="Color" icon="color"><p className="panel-placeholder">Color controls are a future editor feature.</p></PanelChrome>);
...
function ToolStrip() {
  ...
  return <nav className="tool-strip" aria-label="Tool strip">{tools.map(([icon, label, selected]) => <IconButton key={icon} label={`${label} (not available yet)`} icon={icon} selected={selected} disabled />)}</nav>;
}
