...
function BrushOptions({ editor, dispatch }: { readonly editor: EditorSessionSnapshot | null; readonly dispatch: (action: EditorSessionAction) => EditorActionResult | void }) {
  const settings = editor?.brushSettings; if (!settings) return null;
  const control = (label: string, value: number, update: (value: number) => EditorSessionAction, maximum: number, step: number) => <label className="brush-option" key={label}><span>{label}</span><input aria-label={`Brush ${label}`} type="number" min={label === "Size" ? 1 : 1} max={maximum} step={step} value={label === "Size" ? value : Math.round(value * 100)} onChange={event => { const numeric = Number(event.target.value); if (Number.isFinite(numeric)) dispatch(update(label === "Size" ? numeric : numeric / 100)); }} />{label !== "Size" && <em>%</em>}</label>;
  return <div className="brush-options">{control("Size", settings.diameter, value => ({ type: "set-brush-settings", settings: { diameter: Math.max(1, Math.min(2000, value)) } }), 2000, 1)}{control("Hardness", settings.hardness, value => ({ type: "set-brush-settings", settings: { hardness: Math.max(0, Math.min(1, value)) } }), 100, 1)}{control("Opacity", settings.opacity, value => ({ type: "set-brush-settings", settings: { opacity: Math.max(0, Math.min(1, value)) } }), 100, 1)}{control("Flow", settings.flow, value => ({ type: "set-brush-settings", settings: { flow: Math.max(0, Math.min(1, value)) } }), 100, 1)}{control("Spacing", settings.spacing, value => ({ type: "set-brush-settings", settings: { spacing: Math.max(0.01, Math.min(1, value)) } }), 100, 1)}</div>;
}
