...
export class RasterAsset {
  ...
  /** Sparse, stable metadata enumeration for renderer scheduling. No pixel buffers are copied or exposed. */
  allocatedTiles(): readonly RasterTileInfo[] { return [...this.#tiles.keys()].map(id => { const [x, y] = id.split(",").map(Number); return this.tileInfo({ x, y }); }).sort((first, second) => first.y - second.y || first.x - second.x); }
